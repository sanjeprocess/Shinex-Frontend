  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold">Customers / Plants</h2>
        <div className="flex items-center gap-2">
          <SearchInput value={q} onChange={setQ} placeholder="Search plants by code, name or contact..." />
          <button className="bg-[#2F6F5E] text-white px-3 py-1 rounded-md" onClick={onOpenCreate}>Add Plant</button>
        </div>
      </div>
      ...
      <ConfirmDialog open={!!confirm} title="Delete Plant" message={`Delete ${confirm}?`} onConfirm={onDeleteConfirm} onCancel={()=>setConfirm(null)} />
    </div>
  )
}
